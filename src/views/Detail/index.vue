          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd :class="{active:spuSaleAttrValue.isChecked==='1'}" @click="changeCheck(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)" v-for="(spuSaleAttrValue,index) in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id">
...
export default {
  ...
  methods:{
    ...
    async addCart(){
      ...
      try {
        let result = await this.$store.dispatch('addOrUpdateCart',{skuId,skuNum});
        if(result==='ok'){
          alert('添加购物车成功');
