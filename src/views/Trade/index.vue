<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address,index) in userAdressList" :key="address.id">
        <span class="username" :class="{selected:address.isDefault==='1'}">{{address.consignee}}</span>
        <p @click="changeDefault(address,userAdressList)">
    ...
    <div class="sub clearFix">
      ...
      <a href="javascript:;" class="subBtn" @click="orderSubmit">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      userAdressList:state=>state.Trade.userAdressList,
      tradeInfo:state=>state.Trade.tradeInfo
    }),
    ...mapGetters(['detailArrayList']),
    defaultAddress(){
      return this.userAdressList.find(item=>item.isDefault==='1') || {}
    }
  },
  mounted(){
    // 获取用户地址信息
    this.getUserAdressInfo()
    // 获取用户商品清单数据
    this.getTradeInfo()
  },
  methods:{
    ...
    changeDefault(address,userAdressList){
      userAdressList.forEach(item => {
        item.isDefault='0'
      });
      address.isDefault='1'
    },
  ...
  //     deep: true,
  //     handler(){
  //       this.defaultAddress = this.userAdressList.find(item => item.isDefault==='1');
  //     }
  //   }
  // }
};
